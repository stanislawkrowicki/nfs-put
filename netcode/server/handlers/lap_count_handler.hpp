...
public:
    static void handle(const PacketBuffer &payload, const size_t size, ClientHandle &client, TCPServer *server) {
        ...
        // Trigger race end countdown if someone reaches LAPS_IN_RACE laps
        if (laps >= LAPS_IN_RACE && !server->raceEndCountdownActive.exchange(true)) {
            ...
            std::thread([server]() {
                while (true) {
                    ...
                    const int elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - server->raceEndStartTime)
                            .count();
                    const int remaining = RACE_END_TIMEOUT - elapsed;
